...
function app_fetch(params) {
    var remote, remote_index, dest;

    function fetch_from(url){
        try { 
            remote = base.arangoConnection(url);
        } catch(err) {
    ...
    remote_index = remote.name+'_index';
    remote.query
    .for("u").in(remote_index)
    .filter("u.name=='"+remote.name+"' && u.base=='"+remote.basedir+"'")
    .limit(1).return("u").exec().then(function(res){
        
        remote.edge.list(remote_index,res._id,'out').then(function(res){
            res.edges.forEach(function(edge){
                remote.document.get(edge._to).then(function(data){
                    data = data.content;
                    try {
                        if(base.isBinary(data,edge.file)) {
                            console.log("binary %s (%s bytes)", edge.file, data ? data.length : 0);
                            fs.writeFileSync(basedir+edge.file,new Buffer(data,'base64').toString('binary'),'binary');
                        } else {
                            console.log("ascii %s (%s bytes)", edge.file, data ? data.length : 0);
                            fs.writeFileSync(basedir+edge.file,data,'utf-8');
                        } 
                    } catch(err) { 
                        throw ("Failed to write " + edge.file + ": " + err.message); 
                    }
                },function(err){ 
                    console.log(err) 
                });  
...
function app_push(params){
    ...
    var remote_index = remote.name + '_index';
    /* get remote index */
    remote.edge.list(remote_index,index._id,'out').then(function(res) {
        /* todo: compare local with remote index */
        push_to_remote(res.edges);
    },function(err) {
        if(params.create) {
            /* create remote index */
            remote.collection.create(remote_index,{type:3})
            .then(function(res){
                return remote.document.create(remote_index);
            })
            .then(function(res){
                /* store index id */
                index._id = res._id;
                ...
                console.log("Create collection:", remote.name);
                return remote.collection.create(remote.name);
            })
            .then(function(res){
    ...
    function push_to_remote(edges) {
        ...
        function write_remote_index(){
            ...
            files.forEach(function(file){
                if(!indexed || !(index.files[file]._id in indexed) )
                    indexFile(file);
            }); 
