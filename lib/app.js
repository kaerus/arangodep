...
function app_reset(params){
    ...
    if(params.collection) {
        remote.collection.delete(remote.name).then(function(){
        ...
        remote.simple.example({index:remote.name}).then(function(res){
            res.result.forEach(function(doc){
                remote.document.delete(doc._id).then(function(){  
...
function app_fetch(params) {
    ...
    remote.simple.first({index:remote.name}).then(function(res){
        index = base.extend(true,{remote:params.remote|| index.remote},res.document);
        ...
        console.log("Downloading %s files", num_files);

        var dirname;
        Object.keys(index.files).forEach(function(file){
            num_files--;
            remote.document.get(index.files[file]._id).then(function(doc){ 
                ...
                if(doc.headers && doc.headers['content-type'] === 'base64') {
                ...
                console.log("%s (%s)", doc.file, base.dataSize(data));
                try {        
                ...
                console.log("Failed to fetch %s:", file, err);
                num_files--; 
            });  
        });
        ...
        function when_done(){
            if(num_files > 0) {
                setTimeout(when_done,250);
            } else {
                if(update_index()) {   
                    console.log("Updated local index");
                } else {
                    console.log("Failed to update local index");
                }    
            }
        }
        setTimeout(when_done,250);
...
function app_push(params,args){
    ...
    remote.document.get(index._id).then(function(res) {
        ...
        if(params.create) {
            /* create remote index */
            console.log("creating index:", remote.name);
            remote.document.create({index:remote.name},{createCollection:true})
            .then(function(res){
                /* store index id */
                index._id = res._id;
                index._rev = res._rev;
...
function app_init(params,args){
    ...
    function create_index() {
        index = {remote:params.remote,files:{},modules:{},routes:{},created:now}
        try {
            fs.writeFileSync(basedir+PSEP+'.index', JSON.stringify(index,null,2));
