...
/* add commands to root context */
var context = Program.$("app","application");
...
function app_reindex(params){
	...
	.join(reindexed).then(finalize).catch(function(error){
            if(error) console.log("Reindex error: %j", error);
	    else finalize();
	});
...
function app_add(params,args){
    ...
    if(stat.isDirectory()){
        ...
        traverse.call(this,source,function(file) {
...
function traverse(dir,callback,subdir){
    ...
    files.forEach(function(file) {
        ...
        /* traverse subdirs */
        if(stat.isDirectory()) 
            traverse.call(this,realpath,callback,basepath);
        else if(stat.isFile())
