...
function app_reindex(params){
    ...
    console.log("Attempting to rebuild index", index.remote);
    Object.keys(index.files).forEach(function(file){
        reindex_file(remote,file);
    });
}

function reindex_file(remote,file){
    return remote.simple.example({file:file}).then(function(res){
        if(res.count == 1){
            if(index.files[file]._id === res.result[0]._id) {
                console.log("%s _id ok",file);
                return;
            }

            console.log("Indexed %s with _id", file, res.result[0]._id);

            index.files[file]._id = res.result[0]._id;
        } else if(res.count > 1){
            index.files[file]._id = res.result[res.count-1]._id;
            console.log("Indexed %s with _id %s", file, res.result[res.count-1]._id);
