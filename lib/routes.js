...
context.$(
    route_create,"create","Add route",
    base.REMOTE_OPTION,
    {"url":{"string":""},"?":"Route url"},
    {"action":{"string":""},"?":"Action controller"},
    {"constraint":{"string":""},"":null,"?":"URL constraints"},
    {"controller":{"string":""},"":null,"?":"Specific controller"},
    {"prefix":{"string":""},"":null,"?":"Prefix controller"},
    {"do":{"string":""},"":null,"?":"Do controller method"}
);

context.$(
    route_delete,"remove", "Delete route",
    base.REMOTE_OPTION,
    {"id":{"string":""},"?":"Route id"}
);
...
function route_create(params){
    ...
    if(params.constraint){
        try {
            params.constraint = JSON.parse(params.constraint);
        } catch(e) {
            try {
                params.constraint = JSON.parse('{'+params.constraint+'}');
            } catch(e) {
                throw new Error("Invalid constraint" + e.message);
            }
        }

        if(typeof route.url === 'string')
            route.url = {match:doc.url,constraint:params.constraint};
        else 
    ...
    if(params.prefix) {
        if(typeof route.action === 'string')
            doute.action = {prefixController:params.action};
        else {
    ...
    remote.document.create("_routing",route,function(err,ret){
