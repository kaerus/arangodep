...
function route_add(params){
    ...
    if(params.options && route.action) {
        ...
        if(params.id) {
            remote.document.patch(params.id,params.middleware ? {middleware:[route]} : route,function(err,ret){
            ...
            if(!route.url || !route.action) console.log("Missing route action or url");
                ...
                remote.use(":_routing").document.create(params.middleware ? {middleware:[route]} : route,function(err,ret){
